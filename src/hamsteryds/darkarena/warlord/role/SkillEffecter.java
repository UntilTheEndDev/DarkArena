...
public class SkillEffecter {
	public static void effect(ItemStack item, Player player) {
		...
		if(material==Material.SULPHUR) return;
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					if(counter>=4*5) {
						...
						player.teleport(loc);
						player.setHealth((player.getHealth()+player.getMaxHealth()*0.25)>=player.getMaxHealth()?player.getMaxHealth():(player.getHealth()+player.getMaxHealth()*0.25));
